...
def main(_):
    ...
    train_word=np.load('./data/npy/nlpcc2016_train_word.npy')
    train_label=np.load('./data/npy/nlpcc2016_train_label.npy')
    train_length=np.load('./data/npy/nlpcc2016_train_length.npy')
    print('read cws train data......')
    train_cws_word=np.load('./data/npy/nlpcc2016_cws_word.npy')
    train_cws_label=np.load('./data/npy/nlpcc2016_cws_label.npy')
    train_cws_length=np.load('./data/npy/nlpcc2016_cws_length.npy')
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            with tf.variable_scope('ner_model',reuse=None,initializer=initializer):
                m=Nlpcc_model.TransferModel(setting,tf.cast(embedding,tf.float32),adv=True,is_train=True)
                m.multi_task()
            ...
            for one_epoch in range(setting.num_epoches):
                ...
                np.random.shuffle(list(temp_order_cws))
                for i in range(len(temp_order)//setting.batch_size):
                    #print "enter the first cicle....."
                    for j in range(2):
                        if j==0:
                            temp_word = []
                            temp_label = []
